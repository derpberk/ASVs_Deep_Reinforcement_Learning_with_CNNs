...
from utils.Max_Heap import Max_Heap
...
class Prioritised_Replay_Buffer(Max_Heap, Deque):
    ...
    def separate_out_data_types(self, experiences):
        ...
        states = torch.from_numpy(np.stack([e.value[self.indexes_in_node_value_tuple["state"]] for e in experiences], axis=0)).float().to(self.device)
        actions = torch.from_numpy(np.stack([e.value[self.indexes_in_node_value_tuple["action"]] for e in experiences], axis=0)).float().to(self.device)
        rewards = torch.from_numpy(np.stack([e.value[self.indexes_in_node_value_tuple["reward"]] for e in experiences], axis=0)).float().to(self.device)
        next_states = torch.from_numpy(np.stack([e.value[self.indexes_in_node_value_tuple["next_state"]] for e in experiences], axis=0)).float().to(self.device)
        dones = torch.from_numpy(np.stack([e.value[self.indexes_in_node_value_tuple["done"]] for e in experiences], axis=0)).float().to(self.device)
